...
<style>
  ...
  *{box-sizing:border-box; -webkit-tap-highlight-color:transparent}
  ...
  .slide.list{padding-bottom:96px}
  .artlist{display:flex;flex-direction:column}
  .artrow{display:flex;align-items:center;gap:10px;padding:13px 2px;border-bottom:1px solid var(--line);
    cursor:pointer}
  .artrow:last-child{border-bottom:none}
  .artrow .dot{width:6px;height:6px;border-radius:50%;background:var(--brand);flex:0 0 auto}
  .artrow .t{flex:1;font-size:14.5px;font-weight:600;line-height:1.4;
    white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .artrow .arrow{color:var(--muted);font-size:14px;flex:0 0 auto}
...
function buildDeck(md, report){
  ...
  const list = $('section'); list.className = 'slide list';
  let listHtml = '<h2>📋 오늘의 기사 목록</h2>';
  listHtml += '<div class="artlist">' + slides.map((s, i) =>
    '<div class="artrow" data-idx="' + i + '">'
    + '<span class="dot"></span>'
  ...
  list.querySelectorAll('.artrow').forEach(row => {
    row.onclick = () => { deck.scrollLeft = (Number(row.dataset.idx) + 2) * window.innerWidth; };
  });
  ...
  function updateProgress(){
    ...
    label.textContent = i === 0 ? '표지' : i === 1 ? '목록' : ((i - 1) + ' / ' + slides.length);
  }
