...
/* ---------- 음성으로 듣기 ----------
   1순위: 미리 생성해 둔 자연스러운 음성 파일(audio/<date>/<n>.mp3)
   2순위(파일 없는 옛날 날짜 등): 브라우저 내장 TTS로 자동 대체 */
...
function audioUrl(date, n){ return './audio/' + encodeURIComponent(date) + '/' + n + '.mp3'; }
function slideSpeechText(s){
  const parts = [s.title];
  s.blocks.forEach(b => parts.push(b.li != null ? b.li : b.p));
  return parts.join('. ');
}
function speakSynthFallback(i){
  if(!synth){ stopTTS(); return; }
  synth.cancel();
...
function playAudioSlide(i){
  const el = new Audio(audioUrl(currentReportDate, i + 1));
  ttsAudioEl = el;
  let settled = false;
  el.onended = () => { if(ttsPlaying) speakIndex(i + 1); };
  el.onerror = () => { if(!settled){ settled = true; speakSynthFallback(i); } };
  el.play().then(() => { settled = true; }).catch(() => {
    if(!settled){ settled = true; speakSynthFallback(i); }
  });
}
...
function startTTS(){
  if(!currentSlides.length || (!synth && typeof Audio === 'undefined')) return;
  ttsPlaying = true;
...
function stopTTS(){
  ...
  if(ttsAudioEl){ ttsAudioEl.onended = null; ttsAudioEl.onerror = null; ttsAudioEl.pause(); ttsAudioEl = null; }
  const btn = document.getElementById('ttsbtn');
